...
// Note: Some addresses and coordinates are approximations. Always verify for accuracy.
  ...
  // Write me after this comment :
  confirmClearAll(): void {
    if (confirm('Are you sure you want to clear all refund requests? This action cannot be undone.')) {
      this.clearAllRefunds();
    }
  }

  clearAllRefunds(): void {
    this.loading = true;
    this.refundService.clearAllRefunds().subscribe({
      next: () => {
        this.refundRequests = [];
        this.showMessage('All refund requests have been cleared successfully.', 'success');
        this.loading = false;
      },
      error: (error) => {
        console.error('Error clearing refund requests:', error);
        this.showMessage('Error clearing refund requests. Please try again.', 'error');
        this.loading = false;
      }
    });
  }
